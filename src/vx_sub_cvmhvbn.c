/** vx_sub_cvmhvbn.c - Query interface to GoCAD voxet volumes and GTL. Supports
...
int vx_setup(const char *data_dir)
{
  ...
  char HR_PAR[CMLEN];
  sprintf(HR_PAR, "%s/CVMHB-Ventura-Basin.vo", data_dir);
...
******/
  ...
  if (entry->coor_utm[1] < 10000000) {
    ...
    if(enhanced == True) {
      ...
      if(cvmhvbn_debug) { fprintf(stderr," FOUND cvmh surface -- %lf\n", surface); }
      if (surface < -90000.0) {
...
if(_debug) fprintf(stderr," ===FINAL: before calling >>> depth %lf surface %lf utm2 %lf coor2 %lf \n", depth, surface, entry->coor_utm[2], entry->coor[2]);
    ...
    if(cvmhvbn_debug) { fprintf(stderr,"Looking into (HR)>>>>>> entry->coor(%lf %lf %lf)\n",
                                                        entry->coor[0], entry->coor[1], entry->coor[2]); }
...
if(cvmhvbn_debug) { fprintf(stderr,"  >FOUND IN HR but NODATA>>>>>> j(%d) gcoor(%d %d %d) vp(%f) vs(%f)\n",j, gcoor[0], gcoor[1], gcoor[2], entry->vp, entry->vs); }
         }
         entry->data_src = VX_SRC_HR;
         entry->depth = surface - entry->coor_utm[2];

if(_debug) fprintf(stderr," === Woohoo depth(%lf) \n", entry->depth);
if(cvmhvbn_debug) { fprintf(stderr,"  >DONE(In HR)>>>>>> j(%d) gcoor(%d %d %d) vp(%f) vs(%f)\n",j, gcoor[0], gcoor[1], gcoor[2], entry->vp, entry->vs); }
...
//--  if it turns out to be do_bkg, that means return NO DATA 
  ...
  if(cvmhvbn_debug) { fprintf(stderr,"   DONE(HR)>>>>>> j(%d) gcoor(%d %d %d) vp(%f) vs(%f) rho(%f)\n",j, gcoor[0], gcoor[1], gcoor[2], entry->vp, entry->vs, entry->rho); }
...
if(cvmhvbn_debug) fprintf(stderr,"DONE --- vx_getcoord_private\n");
...
{
if(cvmhvbn_debug) fprintf(stderr,"CALLING -- vx_getsurface_private\n");
  int gcoor[3];
  double SP[2],SPUTM[2];
  int j;
  vx_entry_t entry;
...
if(cvmhvbn_debug) fprintf(stderr," SURFACE: in topo gcoor(%d,%d,%d)\n",gcoor[0],gcoor[1],gcoor[2]);
	...
	while (!flag) {
if(cvmhvbn_debug) fprintf(stderr," SURFACE: (%d)LOOPING in here %lf\n", num_iter,entry.coor[2]);
...
// go down to see where the actual surface for this point is
	  ...
	  if ((entry.vp < 0.0) || (entry.vs < 0.0)) {
	      ...
	      *surface = p0_NO_DATA_VALUE;
              flag=1;
if(cvmhvbn_debug) fprintf(stderr,"SURFACE: HIT the bottom set to NO_DATA_VALUE\n");
              } else { 
...
if(cvmhvbn_debug) fprintf(stderr,"SURFACE: LOOPING checked %lf\n", entry.coor[2]);
	}
if(cvmhvbn_debug) fprintf(stderr,"SURFACE: LOOPING is over %lf\n", entry.coor[2]);
      } else {
	do_bkg = True;
      }
    }
  ...
  if (do_bkg) {
    *surface = p0_NO_DATA_VALUE;
if(cvmhvbn_debug) fprintf(stderr,"SURFACE: do_bkg set to NO_DATA_VALUE\n");
  }
