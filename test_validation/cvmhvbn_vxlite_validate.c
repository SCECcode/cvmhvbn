/*
 * @file cvmhvbn_vxlite_validate.c
 * @brief test with a full set of validation points
 * @author - SCEC
 * @version 1.0
 *
 * Tests the CVMHVBN as vx_lite_cvmhvbn would and recreate a
 * new input dataset that includes depth from cvmh topography.
 *
 *
 *  ./cvmhvbn_vxlite_validate -m cvmhvbn/model/dir -f validation/dir/CVMHB-Ventura-Basin.dat
 *
 */
...
FILE *_process_datfile(char *fname) {
  ...
  if (fp == NULL) {
    fprintf(stderr,"CVMHVBN_VALIDATE_VXLITE: FAIL: Unable to open the validation data file %s\n", fname);
    exit(1);
  }
  /* read the title line */
  if (fgets(dat_line, 1028, fp) == NULL) {
    fprintf(stderr,"CVMHVBN_VALIDATE_VXLITE: FAIL: Unable to extract validation data file %s\n", fname);
    fclose(fp);
    exit(1);
  }
...
// X,Y,Z,tag61_basin,vp63_basin,vs63_basin
  while(p != NULL)
  {
    if(validate_debug) { printf("CVMHVBN_VALIDATE_VXLITE:'%s'\n", p); }
    if(strcmp(p,"X")==0)
...
/* Usage function */
void usage() {
  printf("     vx_cvmhvbn_lite_valiate - (c) SCEC\n");
  ...
  printf("\tusage: vx_cvmhvbn_lite_validate [-d] -m modeldir -f file.dat\n\n");
  printf("Flags:\n");
  printf("\t-f point.dat\n\n");
  printf("\t-d enable debug/verbose mode\n\n");
  printf("\t-m path to cvmhvbn model directory \n\n");
  exit (0);
}
...
CVMHVBN_VALIDATE_VXLITE:356000.000000,3754000.000000,-100.000114
CVMHVBN_VALIDATE_VXLITE: dat.vs(-99999.000000),dat.vp(1480.000000)
CVMHVBN_VALIDATE_VXLITE:   ret vs:(-1.000000) ret vp:(-1.000000)
**/
                     ...
                     if (!_compare_double(entry.vs, -1.0) && !_compare_double(entry.vp, -1.0) &&
                       ...
                       mmcount++;  // just -1 vs -99999
                         ...
                         fprintf(stderr,"\nCVMHVBN_VALIDATE_VXLITE:Mismatching -\n");
                         fprintf(stderr,"CVMHVBN_VALIDATE_VXLITE:%lf,%lf,%lf\n",dat.x, dat.y, dat.z);
                         fprintf(stderr,"CVMHVBN_VALIDATE_VXLITE: dat.vs(%lf),dat.vp(%lf)\n",dat.vs, dat.vp);
                         fprintf(stderr,"CVMHVBN_VALIDATE_VXLITE:   entry vs:(%lf) entry vp:(%lf)\n",entry.vs, entry.vp);
        ...
        fprintf(stderr,"CVMHVBN_VALIDATE_VXLITE: %d mismatch out of %d (mmcount %d)\n", mcount, tcount,mmcount);
        fprintf(stderr,"CVMHVBN_VALIDATE_VXLITE: good with matching values(%d) \n",okcount );
